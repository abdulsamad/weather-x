...
function WeeklyStats() {
  const { timeFormat, next7Days, next48Hours, loading, place, unit, alert } = useAppContextState();
  ...
  const next48HoursTempData = next48Hours.map(({ dt, temp }) => {
    const time =
      timeFormat === 12 ? dayjs(dt * 1000).format('hh:mm A') : dayjs(dt * 1000).format('HH:mm');
  ...
  const nextWeekTempData = next7Days.map(({ dt, temp: { min, max, day, night } }) => {
  ...
  return (
    <div
      ...
      <h2 className='uppercase text-center text-lg font-bold my-5'>Temperature Next Week</h2>
      <div className='my-5'>
      ...
      <h2 className='uppercase text-center text-lg font-bold my-5'>Temperature Next 48 Hours</h2>
      ...
      <h2 className='uppercase text-center text-lg font-bold my-5'>UV Index Next Week</h2>
      <div className='my-5'>
        <ResponsiveContainer height={200}>
