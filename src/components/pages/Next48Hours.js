...
function Next48Hours() {
  const { unit, next48Hours, timeFormat, loading, place, alert } = useAppContextState();
  ...
  return (
    <div
      ...
      <div className='vertical-scroll h-full w-full overflow-auto container md:px-2 mx-auto'>
        ...
        {next48Hours &&
          next48Hours.map(({ weather, wind_speed, wind_deg, temp, dt }, index) => (
            <Fragment key={dt}>
                      ...
                      {(props) => (
                        <div
                          ...
                          <div className='w-1/2 flex flex-col items-start my-1'>
                            ...
                            <span className='text-lg capitalize'>{weather[0].description}</span>
                          </div>
